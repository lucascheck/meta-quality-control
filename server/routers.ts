...
import { decryptToken, encryptToken, getPhoneNumberDetails, listPhoneNumbers as fetchPhoneNumbers, listTemplates as fetchTemplates, normalizePhoneNumber, sendTemplate } from "./meta";
  ...
  connections: router({
    ...
    sync: owner.input(z.object({ connectionId: z.number() })).mutation(async ({ ctx, input }) => {
      ...
      for (const item of numberResponse.data || []) {
        const detail = await getPhoneNumberDetails(config, String(item.id)).catch(() => ({}));
        const normalized = normalizePhoneNumber({ ...item, ...detail }); const metaId = normalized.metaId; const rating = normalized.qualityRating; const limit = normalized.messagingLimit;
        ...
        if (existing[0]) { await db.update(phoneNumbers).set({ displayPhoneNumber: normalized.displayPhoneNumber, verifiedName: normalized.verifiedName, status: normalized.status, qualityRating: rating, qualityRaw: normalized.qualityRaw, codeVerificationStatus: normalized.codeVerificationStatus, throughputLevel: normalized.throughputLevel, messagingLimit: limit, lastSyncedAt: new Date() }).where(eq(phoneNumbers.id, existing[0].id)); if (existing[0].qualityRating !== rating) await db.insert(qualityHistory).values({ userId: ctx.user.id, phoneNumberId: existing[0].id, qualityRating: rating }); }
        else { const [created] = await db.insert(phoneNumbers).values({ userId: ctx.user.id, connectionId: connection.id, metaId, displayPhoneNumber: normalized.displayPhoneNumber, verifiedName: normalized.verifiedName, status: normalized.status, qualityRating: rating, qualityRaw: normalized.qualityRaw, codeVerificationStatus: normalized.codeVerificationStatus, throughputLevel: normalized.throughputLevel, messagingLimit: limit, lastSyncedAt: new Date() }).$returningId(); await db.insert(qualityHistory).values({ userId: ctx.user.id, phoneNumberId: created.id, qualityRating: rating }); }
      }
